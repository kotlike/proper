...
*/
    find: function(q, context) {
        var ele = [], tempNode;
        // We do not need this check as we use Sizzle for this version of the library
...
// helper function that walks a DOM branch
// fixing self-contained attributes on DOM elements
// and constructs a unique array of all attributes
// present in this sub-tree
function fixGatherAttributes(node, gather){
  var attrs = node.attributes,
    above = arguments[2] || [],
    toPush = [],
    ch = node.firstChild,
    l = attrs && attrs.length ? attrs.length : 0,
    a, i, j;
  gather = !!gather;
  if(node && node.nodeType && node.nodeType === 1){
    ...
    for(i=0, l; i<l; ++i){
      ...
      // fix attribute
      if(a.nodeValue === '' || a.nodeValue === a.nodeName) xui(node).attr(a.nodeName, a.nodeName);

      if(gather){

        for(j=0; j < above.length; ++j)
          if( above[j] == a.nodeName)
            break;

        // add the attribute if it is not already mentioned above
        // or if it still empty(root) - start the party
        if(!above.length || above.length == j) toPush[toPush.length] = a.nodeName;
      }
    }

    if(gather) above.push.apply(above, toPush);

    while(ch){
      fixGatherAttributes(ch, gather, above);
      ch = ch.nextSibling;
    }
  }
    ...
    html: function(location, html) {
        ...
        if (arguments.length == 0) {
            ...
            this.each(function(el) {
              // normalize all attributes of the form attr to look like attr="attr"
              fixGatherAttributes(el);
              i.push(el.innerHTML);
            });
            return i.join('').replace(/<(\/?\w+)([^>]*)?>/g, function(){ // Make sure that HTML tags and attribute names are lowercase cross-browser
              ...
              // TODO: this approach assumes that there is no '=' in attribute values - BAD
                attribs = arguments[2] ? arguments[2].split('=') : []; // Now as all attributes are normalized it must be valid to split them by '='

              for(i=0, l = attribs.length; i < l; ++i){
                attribs[i] = attribs[i].split(' ');
                if(i > 0){
                  n = attribs[i-1].pop(); // this is the CURRENT attributes name
                  attribs[i-1] = attribs[i].pop(); // save the name of the NEXT attribute
                  v = (i == attribs.length - 1) ? attribs[i-1] : attribs[i].join(''); // this is the value of the CURRENT attribute
                  attribs[i].push(attribs[i-1]); // push back the name of the NEXT attribute
                  if(!isSizzleAttr.test(n)){ // filter out attributes internally used by Sizzle
                    parts += ' ' + n.toLocaleLowerCase() + '=' + v;
                  }
                }
              }
              return '<' + parts + '>';
...
function _createResponder(element, eventName, handler) {
    ...
    var responder = function(event) {
        ...
        // normalize event object before sending it to the handler
        fixEvent(event);
...
function fixEvent(event) {
  // check if Target is a TextNode and if so reset target to parent
